...
public final class SalvationBeamAbility {
    ...
    public void tryFireBeam(Player player) {
        int id = player.getEntityId();
        if (hitCounter.get(id) < config.salvationHitsRequired) return;

        long now = System.currentTimeMillis();
        if (now - beamCooldown.get(id) < config.beamCooldownMs) return;
    ...
    public boolean canFireBeam(Player player) {
        int id = player.getEntityId();
        long now = System.currentTimeMillis();
        return hitCounter.get(id) >= config.salvationHitsRequired
                && (now - beamCooldown.get(id)) >= config.beamCooldownMs;
    }
    ...
    private void fireSalvationBeam(Player player) {
        ...
        List<LivingEntity> targets = findBeamTargets(
                world, eye, origin, direction, limit, raySize, maxPierce, player
        );
    ...
    private void spawnLavaTrail(World world, Vector origin, Vector direction, double length) {
        ...
        int maxLava = Math.max(1, MAX_PARTICLES / 2);
        int count = Math.clamp((int) (length * config.beamParticlesPerMeter * LAVA_DENSITY_FACTOR), 1, maxLava);
        ...
        double halfDx = direction.getX() * step * 0.5;
        double halfDy = direction.getY() * step * 0.5;
        double halfDz = direction.getZ() * step * 0.5;
